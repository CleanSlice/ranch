...
- [ ] T005 Run `cd api && bun run migrate`, rename the generated folder to `api/prisma/migrations/20260914120000_agent_peer_delegation/`, prepend `-- Additive: new tables AgentPeer (directed peer connection + pair credential + card snapshot) and AgentDelegation (audit row per delegated task). Safe on an existing database.`, confirm `bun run generate` succeeds (depends on T004)
