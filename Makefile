...
k3d: ## Create/start local k3d cluster
	...
	@KUBECONFIG=$(KUBECONFIG_LOCAL) kubectl apply -f k8s/templates/rlm-job-rbac.yaml 2>/dev/null || true
	@KUBECONFIG=$(KUBECONFIG_LOCAL) kubectl apply -f k8s/templates/rlm-job-networkpolicy.yaml 2>/dev/null || true
