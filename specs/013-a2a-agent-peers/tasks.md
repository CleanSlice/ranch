...
- [ ] T056 [US4] Wire the emitter into `DelegationService` in `api/src/slices/agent/peer/domain/delegation.service.ts`: on `run`, `const turn = hub.findActiveTurn(callerAgentId)`; store `turnId`/`clientId` on the row when known; `onStart` → `hub.sendToClient(turn.clientId, callerAgentId, { type: 'thinking', clientId: turn.clientId, turnId: turn.turnId, ts: Date.now(), step: buildDelegationStep(row, skills, 'start') })`; `onFinish` → same with `'finish'`; when `turn` is null push nothing and log at debug `no active turn for <agentId>, delegation step not shown`; never send `done: true` (depends on T049, T054)
