...
- **FR-007**: When the platform rejects an access token, the response MUST carry a machine-readable reason that lets a console distinguish "expired" from "invalid" from "none provided". A console MUST answer "expired" and "invalid" by renewing from the session and retrying the request once (an "invalid" token with a live session is the signing-secret-rotation case), and MUST move to the session-ended state only when the renewal itself is refused.
