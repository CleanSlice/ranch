/**
 * Placing file change proposals (CLEAN-112) among transcript messages.
 *
 * A proposal shows as its own agent-side bubble carrying one `proposal` part.
 * On reload the transcript page brings the proposals raised in its window;
 * they are inserted by their creation time — before the first message
 * stamped after it — and deduplicated by id so a live card that arrived over
 * the socket and its persisted twin never show twice.
 *
 * Twin file: `app/slices/bridle/utils/proposalMerge.ts` — change them together.
 */
 ...
 * by `createdAt`. Existing bubbles with the same id are kept where they are.
 * `make` builds the bubble for a proposal that is not on screen yet.
 */
export function mergeProposals<T extends { id: string; ts: number }>(
  ...
  for (let i = 0; i < messages.length; i++) {
    ...
    // Everything created before this message goes in front of it.
