...
- **FR-020**: Text-based attachments (TXT, Markdown, CSV, JSON) MUST have their contents read at send time and carried into the message the agent receives, so the agent can act on the file without fetching anything. Content beyond a per-file limit of 100,000 characters MUST be truncated with an explicit notice in place of the removed portion, and the person MUST be told in the compose area when truncation will occur. *Superseded by CLEAN-67 for spreadsheets: workbooks are inlined as a bounded preview (row-boundary cuts, per-sheet omission counts) and read in full through the `query_attachment` tool — see `specs/009-attachment-parse-quality/`.*
...
- **Text extraction limit**: 100,000 characters per file is chosen as a default that comfortably covers ordinary documents while staying well inside what a model turn can carry. It lives with three sibling constants in `api/src/slices/bridle/domain/attachment.constants.ts` (`MAX_EXTRACTED_TEXT_CHARS`, `SPREADSHEET_INLINE_BUDGET_CHARS`, `SPREADSHEET_PREVIEW_ROWS_PER_SHEET`, `MAX_QUERY_CELLS`); since CLEAN-67 spreadsheets use the two preview constants instead of the 100,000-character cap.
