...
describe('list_agent_files / read_agent_file', () => {
  ...
  it('list narrows by prefix and carries kind/editable', async () => {
    const { tool } = harness();
    const all = parse(
      await tool.listAgentFiles({ agentId: 'agent-a' }, undefined, operator()),
    );
    expect(all.count).toBe(3);
    const skills = parse(
      await tool.listAgentFiles(
        { agentId: 'agent-a', prefix: 'skills' },
        undefined,
        operator(),
      ),
    );
    expect(
      (skills.files as Array<{ path: string }>).map((f) => f.path),
    ).toEqual(['skills/x/SKILL.md']);
  ...
  it('read passes offset and caps the slice, and reports a refusal as an error result', async () => {
    ...
    const res = parse(
      await tool.readAgentFile(
        { agentId: 'agent-a', path: 'SOUL.md', offset: 0, limit: 10_000_000 },
        undefined,
        operator(),
      ),
    );
    ...
    files.readRange.mockRejectedValueOnce(
      new Error('data/photo.png is not a text file'),
    );
    const refused = await tool.readAgentFile(
      { agentId: 'agent-a', path: 'data/photo.png' },
      undefined,
      operator(),
    );
...
describe('write_agent_file / create_agent_file', () => {
  ...
  it('confirm applies exactly the named proposal', async () => {
    ...
    expect(proposals.apply).toHaveBeenCalledWith(
      'prop-1',
      expect.objectContaining({ via: 'tool', actor: 'agent:agent-ops' }),
    );
  });
  ...
  it('create defaults to empty content and proposes with op create', async () => {
    const { tool, proposals } = harness();
    await tool.createAgentFile(
      { agentId: 'agent-a', path: 'notes/todo.md' },
      undefined,
      operator(),
    );
    expect(proposals.propose).toHaveBeenCalledWith(
...
describe('import_agent_files', () => {
  it('fetches the attachment from the chat agent, stages, plans and proposes a set', async () => {
    ...
    const res = parse(
      await tool.importAgentFiles(
        { agentId: 'agent-a', attachmentId: 'att-1' },
        undefined,
        operator(),
      ),
    );
    ...
    expect(proposals.proposeImport).toHaveBeenCalledWith(
  ...
  it('needs a source and refuses a non-https link and a missing attachment', async () => {
    ...
    const http = await tool.importAgentFiles(
      { agentId: 'agent-a', url: 'http://example.com/a.zip' },
      undefined,
      operator(),
    );
    ...
    const missing = await tool.importAgentFiles(
      { agentId: 'agent-a', attachmentId: 'nope' },
      undefined,
      operator(),
    );
  ...
  it('confirming a replace with removals needs confirmRemove', async () => {
    const { tool, proposals } = harness();
    proposals.get.mockResolvedValue(
      pendingRow({
        id: 'prop-set',
        kind: 'set',
        op: 'import',
    ...
    proposals.apply.mockRejectedValueOnce(
      new ProposalNeedsRemoveConfirmError(pendingRow() as never, 3),
    );
    ...
    proposals.apply.mockResolvedValueOnce(
      pendingRow({ id: 'prop-set', status: 'applied', result: { written: 3 } }),
    );
    const done = parse(
      await tool.importAgentFiles(
        {
    ...
    expect(proposals.apply).toHaveBeenLastCalledWith(
      'prop-set',
      expect.objectContaining({ confirmRemove: true }),
    );
  });
});
